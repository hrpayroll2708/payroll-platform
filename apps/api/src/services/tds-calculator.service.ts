...
export class TdsCalculatorService {
  ...
  /**
   * Authoritative FY 2026-27 Slabs (Budget Section 115BAC New Regime)
  ...
  public static calculateTaxProjection(params: {
    ...
    // 1. Total projected gross (including previous employer salary from Form 12B)
    ...
    // 3. Exemptions and Chapter VI-A deductions (exclusive to Old Regime)
    ...
    if (isNewRegime) {
      if (netTaxableIncome <= 1200000) {
        rebate87A = grossTaxLiability; // 100% tax rebate up to ₹12,00,000 taxable income
      } else {
        // Section 87A Marginal Relief: tax payable cannot exceed income exceeding ₹12,00,000
    ...
    // Section 206AA Penalty: Missing or invalid PAN triggers flat 20% minimum tax
    if (isPanMissing) {
    ...
    // 9. YTD Spreading across remaining pay periods
